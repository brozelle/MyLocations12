...
//Draws the text
    ...
    func show(animated: Bool) {
      if animated {
        //Sets up initial state of the view prior to animation
        alpha = 0
        transform = CGAffineTransform(scaleX: 1.3, y: 1.3)
        // Pass the method a closure that describes what happens with the animation
    UIView.animate(withDuration: 0.3,
          ...
          // Sets up state of the view after the animation
          self.alpha = 1
          self.transform = CGAffineTransform.identity
        }
      }
